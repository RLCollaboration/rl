...
tf.app.flags.DEFINE_boolean('render', False, 'Whether to render a display of the environment state.')
...
tf.app.flags.DEFINE_float('epsilon', 0.1, 'The initial exploration factor (epsilon) to be used.')

tf.app.flags.DEFINE_integer('buffer_size', 1000, 'Maximum replay buffer size.')
tf.app.flags.DEFINE_integer('minibatch_size', 50, 'Size of minibatches sampled from replay buffer.')
...
def calculate_loss(minibatch, Q):
    ...
    loss = 0
    for tr in minibatch:
        y = tr.reward + gamma * np.argmax(Q(tr.next_state)) * (1 - tr.is_terminal)
        q = Q(tr.next_state)[tr.action]
        loss += np.square(y - q)
...
def train(env):
    ...
    with tf.Graph().as_default():
        ...
        s = tf.placeholder(name='s', dtype=tf.float32, shape=(None, 10))
        ...
        with tf.train.MonitoredTrainingSession(
            ...
            while not mon_sess.should_stop():
                ...
                action = eps_greedy(env, Q_action)
                ...
                if replay_buffer.full():
                    ...
                    if global_step_value % FLAGS.train_q_freq == 0:
                        ...
                        minibatch = replay_buffer.sample(FLAGS.minibatch_size)

                        # TODO: Calculate loss
                        loss = calculate_loss(minibatch, Q)
...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    env = EnvWrapper(FLAGS.environment)
    train(env)
